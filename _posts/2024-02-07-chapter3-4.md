...
이전 포스팅에서 살펴봤듯이, principal component direction인 $v_m$은 $\mathbf{V}$의 컬럼벡터($\mathbf{S}$의 고유벡터)였다.이는 수학적 귀납법을 사용하여 증명할 수 있다. $m$이 1인 경우는 위에서 확인했으므로, $m=k-1$이 성립할 때 $m=k$도 성립하는지 확인해보자. 
...
$$
\begin{align*}
2v_j^T\mathbf{S}^2v_k - 2\lambda v_j^T\mathbf{S}v_k + \sum_{l=1}^{k-1}c_lv_j^T\mathbf{S}^2v_l &= 0 \\
2(\mathbf{S}v_j)^T\mathbf{S}v_k + c_jv_j^T\mathbf{S}v_j &= 0 \\
2\lambda_j v_j^T\mathbf{S}v_k + c_jv_j^T\mathbf{S}v_j &= 0 \\
c_jv_j^T\mathbf{S}v_j &= 0 \ \text{for} \ \forall j
\end{align*}
$$
...
즉 $\hat{\beta}^{pcr}(M) = \sum_{m=1}^M \hat{\theta}_mv_m$이다. 또한 $\mathbf{Z} = \mathbf{X}\mathbf{V} = \mathbf{U}\mathbf{D}$이므로, $M=p$일 경우 완전히 OLS의 식과 같다. 아래 그림은 prostate 데이터를 이용해 릿지 회귀와 PCR을 비교한 것으로 두 모델이 상당히 유사한 것을 알 수 있다. 차이점은 릿지는 변수를 점진적으로 줄이고, PCR은 완전히 제거해버린다는 점이다. 책에서는 이를 각각 shirinkage and truncation patterns이라고 표현하였다.
