...
계산상의 편의를 위해 $\alpha$를 1로 두고 $\mathbf{u}_{\mathcal{A}}$를 찾아보자. 이는 항등행렬을 이용하여 풀 수 있다. 
...
$$
라 하자(만약 $j \in \mathcal{A}$라면 $$a_j = \mathbf{A}_{\mathcal{A}}$$). 이제 다음 업데이트할 $\hat{\mathbf{\mu}}$와 상관관계를 $\gamma (0 < \gamma  \leq 1)$에 대한 함수로 정의해보자.
...
$$
이고, $\mathbf{u}_{\mathcal{A}}$은 현재 잔차의 단위벡터이므로 둘은 같은 값이다. 또한 $0 < \gamma \leq 1$이므로,
...
이다. 그리디함을 줄이기 위해서는 가능한 작은 스텝사이즈만큼 움직이는 것이 좋을 것이다. 따라서 $\hat{\gamma}$는 위 두 값중 작은 값을 취한다. 즉,
