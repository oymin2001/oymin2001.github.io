...
4. $\mathcal{A}$에 모든 벡터가 포함되지 않았다면, 
   ...
   $$
   를 계산하여 $j \in \mathcal{A}$에 대해 $$\beta_j \leftarrow \beta_j + \hat{\gamma} w_{\mathcal{A}_j} , \ \hat{\mu} \leftarrow \hat{\mu} + \hat{\gamma} \mathbf{u}_{\mathcal{A}}$$로 업데이트를 진행한다. 만약 $N-1$번 이상으로 진행되면 업데이트가 이뤄지지 않으므로 종료한다.
