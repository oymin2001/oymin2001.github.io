...
다음 그림은 prostate cancer example데이터셋을 이용해 릿지회귀를 나타낸 것이다. 빨간 점선으로 칠해진 수직선은 cross-validation을 통해 test error가 가장 낮은 $\lambda$ (df=5.0)를 찾은것이다.
...
다음 그림은 prostate cancer example데이터셋을 이용해 릿지회귀를 나타낸 것이다. x축의 Shirnkage Factor인 $s$는 $\frac{t}{\sum_{j=1}^p \| \hat{\beta_j} \|}$이다. 빨간 점선으로 칠해진 직선은 릿지와 마찬가지로 cross-validation을 통해 찾은 최적의 $s$(=0.36)이다.
