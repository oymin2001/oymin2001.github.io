...
$$
\bar{\mu} = \frac{1}{K}\sum_{k=1}^K \hat{\mu_k}
$$

$$
\begin{align*}
\hat{\Sigma}_T &= \frac{1}{N}\sum_{i=1}^N (x_i-\bar{\mu})(x_i-\bar{\mu})^T \ \text{(total covariance)} \\
\hat{\Sigma}_W &= \frac{1}{N}\sum_{k=1}^K\sum_{g_i = k}(x_i - \hat{\mu}_k)(x_i - \hat{\mu}_k)^T \ \text{(within-class covariance)} \\
\hat{\Sigma}_B &= \frac{1}{N}\sum_{k=1}^KN_k(\hat{\mu}_k - \bar{\mu})(\hat{\mu}_k - \bar{\mu})^T \ \text{(between-class covariance)}
...
$$
\sum_{i=1}^N x_i =\sum_{i:g_i=1}x_i + \sum_{i:g_i=2}x_i =  N_1\hat{\mu}_1 + N_2\hat{\mu}_2, \ \sum_{i=1}^N Y_i = 0, \ \sum_{i=1}^Nx_iY_i = -\frac{N}{N_1}N_1\hat{\mu}_1 + \frac{N}{N_2}N_2\hat{\mu}_2 = N(\hat{\mu}_2 - \hat{\mu_2})
$$
...
$$
\hat{G}(x) = \underset{k\in\mathcal{G}}{\mathrm{argmin}} \frac{1}{N} \| y_k - \hat{f}(x)\|^2 = \underset{k\in\mathcal{G}}{\mathrm{argmin}} \frac{1}{N} \| y_k - \mathbf{X}\mathbf{b}_j^{LR}\|^2
$$


$$
\hat{G}(x) = \underset{k\in\mathcal{G}}{\mathrm{argmin}} \frac{1}{N} \| y_k - \mathbf{X}b_k^{LDA}\|^2 - \mathbf{b}\hat{\Sigma}_B\mathbf{b}
$$
